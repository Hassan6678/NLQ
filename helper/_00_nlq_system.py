...
class NLQSystem:
    ...
    def query(self, nlq: str, table_name: str = "sales_data") -> QueryResult:
        ...
        try:
            ...
            # Reconcile filters generically with intent (adds explicit ones, removes hallucinated ones, fixes time)
            sql = self._reconcile_filters_with_intent(nlq, sql, table_name)

            # Fix incorrect self-joins that cause ambiguous column references
            sql = self._fix_incorrect_self_joins(sql, table_name)

            # Conditionally enforce route <> 'UNK' only when a location dimension is explicitly mentioned
            sql = self._maybe_add_route_exclusion(nlq, sql)

            # Harden growth percentage divisions to avoid NULL/NaN/Inf by rewriting risky patterns
            sql = self._rewrite_growth_divisions_safe(sql)
            ...
            if cached_result:
                ...
                # Ensure cached result is JSON-safe (older cache entries may contain NaN/Inf)
                try:
                    cached_result.data = self._sanitize_result_dataframe(cached_result.data)
                except Exception:
                    pass
            ...
            # Generate a human-readable summary only when we have rows
            try:
                if getattr(result, "row_count", 0) > 0:
            ...
            # Sanitize DataFrame (replace NaN/Inf with None) BEFORE caching or returning to avoid JSON parse issues client-side
            try:
                result.data = self._sanitize_result_dataframe(result.data)
            except Exception:
                pass
            ...
            # Only cache if data does not contain problematic NaN tokens after sanitization
            try:
                if not result.data.isnull().any().any():
                    self.cache.cache_result(sql_to_run, result)
                else:
                    logger.debug("Skipping result cache due to remaining null values (allowed but not cached)")
            except Exception:
                self.cache.cache_result(sql_to_run, result)
    ...
    def _build_rule_based_sql(self, nlq: str, table_name: str) -> Optional[str]:
        """Generic, intent-driven fallback. Extracts intent (dimension, metric, filters, time,
        ordering, limit, and analysis type) and composes SQL from a small set of templates.
        This avoids special-casing and keeps behavior consistent across queries.
        """
        try:
            table_cols = [c.lower() for c in self.db_manager.table_schemas.get(table_name, {}).get("columns", [])]
            if not table_cols:
                return None
            intent = self._extract_intent_from_nlq(nlq, table_cols, table_name)
            if intent is None:
                return None
            sql = self._build_sql_from_intent(intent, table_name)
            logger.info(f"Generated intent-based SQL: {sql}")
            return sql
        except Exception as e:
            logger.warning(f"_build_rule_based_sql failed: {e}")
            return None

    def _extract_intent_from_nlq(self, nlq: str, table_cols: List[str], table_name: str) -> Optional[Dict[str, Any]]:
        """Parse NLQ into a normalized intent structure.
        Returns dict with keys: dimension, metric_mode, filters, year, months, order, limit, analysis.
        - metric_mode: one of {sum_metric, stockout_percent, growth_percent, mto_sum}
        - filters: list of (column, operator, value) tuples; values are normalized strings or ints
        """
        try:
            text = nlq.lower().strip()
            # Normalize plural terms to singulars for easier matches
            plural_map = {
                "territories": "territory", "cities": "city", "areas": "area", "regions": "region",
                "routes": "route", "brands": "brand", "customers": "customer", "distributors": "distributor",
                "skus": "sku", "targets": "target", "sales": "sales"
            }
            for p, s in plural_map.items():
                text = re.sub(rf"\b{p}\b", s, text)

            # Dimension
            candidate_dims = [d for d in ["region","city","area","territory","distributor","route","brand","sku","customer"] if d in table_cols]
            dimension = next((d for d in candidate_dims if re.search(rf"\b{d}\b", text)), None)
            if dimension is None:
                dimension = "region" if "region" in table_cols else next((c for c in candidate_dims if c not in {"month","year"}), None)
            if dimension is None:
                return None

            # Analysis type
            is_growth = self._is_growth_query(text)
            wants_stockout = ("stockout" in text or "stock out" in text or "out of stock" in text) and ("stockout" in table_cols)
            wants_missed_target = any(t in text for t in ["missed target","target missed","target gap","gap to target","mto"]) and ("mto" in table_cols)

            # Metric / mode
            metric = None
            if wants_stockout:
                metric_mode = "stockout_percent"
                metric = "stockout"
            elif is_growth:
                metric_mode = "growth_percent"
                metric = "sales" if "sales" in table_cols else ("mro" if "mro" in table_cols else None)
            elif wants_missed_target:
                metric_mode = "mto_sum"
                metric = "mto"
            else:
                metric_mode = "sum_metric"
                metric = self._choose_metric_from_nlq(text, table_cols) or ("sales" if "sales" in table_cols else None)
            if metric is None:
                return None

            # Time
            year, months = self._extract_periods_from_nlq(text)
            if months and year is None:
                try:
                    year = self.db_manager.get_latest_year_for_month(table_name, max(months))
                except Exception:
            ...
            order_dir = "ASC" if any(t in text for t in asc_terms) else ("DESC" if any(t in text for t in desc_terms) else "DESC")
            limit_n = 1
            m_topn = re.search(r"\b(top|bottom)\s*(\d+)\b", text)
            if m_topn:
                try:
            ...
            # Value filters (region/city/etc.)
            filters: List[Tuple[str,str,Any]] = []
            # Region patterns: north-a, north a, south-b, central region
            m_region_exact = re.search(r"\b(north|south|central|east|west)[\-\s]?([ab])\b", text)
            if m_region_exact and "region" in table_cols:
                val = f"{m_region_exact.group(1).capitalize()}-{m_region_exact.group(2).upper()}"
                filters.append(("region", "=", val))
            else:
                # If explicit token like 'north region'
                m_region_base = re.search(r"\b(north|south|central|east|west)\s+(region|area|territory)\b", text)
                if m_region_base and "region" in table_cols:
                    base = m_region_base.group(1).capitalize()
                    filters.append(("region", "LIKE", f"{base}-%"))

            # City/brand/other value tokens if present in text and in sample values
            try:
                table_info = self.db_manager.table_schemas.get(table_name, {})
                sample_rows = table_info.get("sample_data", []) or []
                known_values = {}
                for r in sample_rows:
                    for d in ["city","area","territory","distributor","route","brand","sku","customer"]:
                        v = r.get(d)
                        if isinstance(v, str):
                            known_values.setdefault(d, set()).add(v.lower())
    ...
    def _build_sql_from_intent(self, intent: Dict[str, Any], table_name: str) -> str:
        """Compose SQL from intent structure using generic templates only."""
        dimension = intent["dimension"]
        metric = intent["metric"]
        metric_mode = intent["metric_mode"]
        year = intent.get("year")
        months: List[int] = intent.get("months") or []
        order_dir = intent.get("order_dir", "DESC")
        limit_n = intent.get("limit", 5)
        filters: List[Tuple[str,str,Any]] = intent.get("filters") or []
        ...
        if metric_mode == "stockout_percent":
            cond = "(stockout = 1 OR stockout = TRUE)"
            return (
                f"SELECT {dim_sql}, "
                f"SUM(CASE WHEN {cond} THEN 1 ELSE 0 END) AS stockout_count, "
                f"COUNT(*) AS total_records, "
                f"(SUM(CASE WHEN {cond} THEN 1 ELSE 0 END) * 100.0 / NULLIF(COUNT(*), 0)) AS stockout_percentage "
                f"FROM {table_name}{where_clause} "
                f"GROUP BY {dim_sql} "
                f"ORDER BY stockout_percentage {order_dir}, stockout_count {order_dir} "
                f"LIMIT {limit_n};"
            )

        if metric_mode == "mto_sum":
            alias_metric = "mto"
            return (
        ...
        if metric_mode == "growth_percent":
            # Build standard month-over-month growth within a year (or cross-year if months suggest)
            latest_month = max(months) if months else None
            if latest_month is None:
                # fallback to latest two periods
                return self._build_growth_sql(nlq_lower="", table_name=table_name, dimension=dimension, metric=metric, year=year, months=months, limit_n=limit_n, order_dir=order_dir)
            # compute previous
            prev_month = 12 if latest_month == 1 else latest_month - 1
            prev_year = (year - 1) if (latest_month == 1 and year is not None) else year
            return (
                f"SELECT {dim_sql}, "
                f"(SUM(CASE WHEN month = {latest_month} THEN {metric_sql} ELSE 0 END) - "
                f" SUM(CASE WHEN month = {prev_month} THEN {metric_sql} ELSE 0 END)) * 100.0 / "
                f" NULLIF(SUM(CASE WHEN month = {prev_month} THEN {metric_sql} ELSE 0 END), 0) AS growth_percentage "
                f"FROM {table_name} "
                f"WHERE " + (f"year = {year} AND " if year is not None else "") + f"month IN ({prev_month}, {latest_month}) "
                f"GROUP BY {dim_sql} "
                f"ORDER BY growth_percentage {order_dir} "
                f"LIMIT {limit_n};"
            )

        # Default sum by dimension
    ...
    def _apply_dynamic_time_logic(self, nlq: str, sql: str, table_name: str) -> str:
        ...
        try:
            ...
            if self._sql_has_time_filters(sql):
                ...
                # If NLQ specifies month/year explicitly for a growth question, align SQL month-year pairs accordingly
                if self._is_growth_query(nlq_lower) and extracted_year is not None and extracted_months:
                    return self._align_growth_periods_with_nlq(sql, extracted_year, extracted_months)
                return sql
    ...
    def _align_growth_periods_with_nlq(self, sql: str, year: int, months: List[int]) -> str:
        """Align growth month-year references inside SQL to the NLQ-specified periods.
        If NLQ gives latest month M in year Y, previous P is M-1 with same Y (unless M=1, then Y-1).
        Fixes mismatches like using 2024 for June when NLQ asks July 2025.
        """
        try:
            if not months:
                return sql
            ...
            if latest_month == 1:
                ...
                prev_month, prev_year = latest_month - 1, year

            updated = sql

            # Normalize CASE/WHERE explicit (month=X AND year=YYYY) and (year=YYYY AND month=X)
            # Latest month -> force year = Y
            updated = re.sub(
                rf"(?is)month\s*=\s*{latest_month}\s*AND\s*year\s*=\s*\d{{4}}",
                f"month = {latest_month} AND year = {year}",
                updated,
            )
            updated = re.sub(
                rf"(?is)year\s*=\s*\d{{4}}\s*AND\s*month\s*=\s*{latest_month}",
                f"year = {year} AND month = {latest_month}",
                updated,
            )

            # Previous month -> force year = prev_year
            updated = re.sub(
                rf"(?is)month\s*=\s*{prev_month}\s*AND\s*year\s*=\s*\d{{4}}",
                f"month = {prev_month} AND year = {prev_year}",
                updated,
            )
            updated = re.sub(
                rf"(?is)year\s*=\s*\d{{4}}\s*AND\s*month\s*=\s*{prev_month}",
                f"year = {prev_year} AND month = {prev_month}",
                updated,
            )

            # WHERE ... year = YYYY AND month IN (a,b)
            in_pair = sorted({prev_month, latest_month})
            in_pat = rf"(?is)year\s*=\s*\d{{4}}\s*AND\s*month\s*in\s*\(\s*{in_pair[0]}\s*,\s*{in_pair[1]}\s*\)"
            if re.search(in_pat, updated):
                if prev_year == year:
                    # Same year, simply set year = Y
                    updated = re.sub(in_pat, f"year = {year} AND month IN ({in_pair[0]}, {in_pair[1]})", updated)
                else:
                    # Cross-year, rewrite to OR clause
                    clause = f"((year = {prev_year} AND month = {prev_month}) OR (year = {year} AND month = {latest_month}))"
                    updated = re.sub(in_pat, clause, updated)

            # If month filters exist but no year filter, inject aligned two-period clause
            if self._sql_has_month_filter(updated) and not self._sql_has_year_filter(updated):
                clause = f"((year = {prev_year} AND month = {prev_month}) OR (year = {year} AND month = {latest_month}))"
                updated = self._inject_filter_clause(updated, clause)

            return updated
        except Exception:
            return sql

    def _dedupe_region_filters(self, sql: str) -> str:
        """Normalize and deduplicate region filters to avoid conflicts like:
        - region = 'South-A' AND LOWER(region) = 'south-a'
        - s.region = 'South-A' AND LOWER(region) = 'south-a'
        Keep a single canonical form: LOWER(region) = '<normalized>'
        """
        try:
            s = sql
            # Find exact equality to 'X-Y' with any alias
            m_eq = re.search(r"(?is)\b([a-zA-Z_][a-zA-Z0-9_]*\.)?region\s*=\s*'([A-Za-z]+-[A-Za-z])'", s)
            m_lower = re.search(r"(?is)\blower\(\s*([a-zA-Z_][a-zA-Z0-9_]*\.)?region\s*\)\s*=\s*'([a-z\-]+)'", s)
            if not m_eq and not m_lower:
                return s
            # Normalize
            normalized = None
            if m_lower:
                normalized = m_lower.group(2)
            elif m_eq:
                normalized = m_eq.group(2).lower()
            if not normalized:
                return s

            # Remove all region equality/like filters first (WHERE/AND forms, with/without LOWER, with/without alias)
            alias_opt = r"(?:[A-Za-z_][A-Za-z0-9_]*\.)?"
            patterns = [
                rf"(?is)\s+and\s+{alias_opt}region\s*=\s*'[^']+'\s*",
                rf"(?is)\s+and\s+lower\(\s*{alias_opt}region\s*\)\s*=\s*'[^']+'\s*",
                rf"(?is)\s+and\s+{alias_opt}region\s+like\s*'[^']+'\s*",
                rf"(?is)\s+where\s+{alias_opt}region\s*=\s*'[^']+'\s*and\s*",
                rf"(?is)\s+where\s+lower\(\s*{alias_opt}region\s*\)\s*=\s*'[^']+'\s*and\s*",
                rf"(?is)\s+where\s+{alias_opt}region\s+like\s*'[^']+'\s*and\s*",
                rf"(?is)\s+where\s+{alias_opt}region\s*=\s*'[^']+'\s*$",
                rf"(?is)\s+where\s+lower\(\s*{alias_opt}region\s*\)\s*=\s*'[^']+'\s*$",
                rf"(?is)\s+where\s+{alias_opt}region\s+like\s*'[^']+'\s*$",
            ]
            for p in patterns:
                s = re.sub(p, " ", s)

            # Clean up double WHERE clauses or trailing WHERE
            s = re.sub(r"(?is)\s+where\s+where\s+", " WHERE ", s)
            s = re.sub(r"(?is)\s+where\s*$", "", s)
            
            # Collapse extra spaces
            s = re.sub(r"\s+", " ", s).strip()
            
            # Use the existing _inject_filter_clause method for proper insertion
            canonical_filter = f"LOWER(region) = '{normalized}'"
            s = self._inject_filter_clause(s, canonical_filter)

            return s
        except Exception:
            return sql

    def _normalize_where_clauses(self, sql: str) -> str:
        """Fix common WHERE/AND placement issues introduced during regex-based removals.

        Examples fixed:
        - "FROM table AND year = 2025 WHERE ..." -> "FROM table WHERE year = 2025 AND ..."
        - Leading/trailing ANDs in WHERE bodies
        """
        try:
            s = sql
            if not s:
            ...
            # Move stray 'AND <cond>' that appear immediately after FROM <table> into WHERE
            # e.g., 'FROM sales_data AND year=2025 WHERE ...' -> 'FROM sales_data WHERE year=2025 AND ...'
            s = re.sub(r"(?is)(FROM\s+[A-Za-z_][A-Za-z0-9_\.]*)(\s+AND\s+)([^\s].*?)(\s+WHERE\s)", r"\1 WHERE \3 \4", s)

            # If there's 'FROM table WHERE' but the WHERE body starts with AND, remove the leading AND
            s = re.sub(r"(?is)\bWHERE\s+AND\s+", " WHERE ", s)

            # Remove 'AND' that immediately precedes GROUP BY / ORDER BY / LIMIT
            s = re.sub(r"(?is)\s+AND\s+(GROUP\s+BY|ORDER\s+BY|LIMIT|HAVING|QUALIFY)\b", r" \1", s)

            # If there are conditions immediately after FROM <table> without a WHERE, insert WHERE
            # e.g., 'FROM sales_data year = 2025 AND month = 7' -> 'FROM sales_data WHERE year = 2025 AND month = 7'
            if re.search(r"(?is)\bfrom\b", s) and not re.search(r"(?is)\bwhere\b", s):
                s = re.sub(
                    r"(?is)(FROM\s+[A-Za-z_][A-Za-z0-9_\.]*)(\s+)(?=(?:lower\(|[A-Za-z_][A-Za-z0-9_\.]*\s*(?:=|<>|like|in\s*\(|between)|year\s*(?:=|in\s*\()|month\s*(?:=|in\s*\())))",
                    r"\1 WHERE ",
                    s,
                )
    ...
    def _enforce_domain_semantics(self, nlq: str, sql: str, table_name: str) -> str:
        ...
        try:
            ...
            # 2) Prefer mto for missed target semantics (never compute mto - anything)
            if any(k in nlq_lower for k in ["missed target", "target missed", "target gap", "gap to target", "mto"]):
                if "mto" in table_cols:
                    ...
                    patterns = [
                        ...
                        # Any variant of mto - x or x - mto -> use mto only
                        (r"(?is)\bmto\s*-\s*[A-Za-z_\"\s]+", "mto"),
                        (r"(?is)[A-Za-z_\"\s]+\s*-\s*mto\b", "mto"),
                        (r"(?is)SUM\s*\(\s*mto\s*-\s*[^\)]+\)", "SUM(mto)"),
                        (r"(?is)SUM\s*\(\s*[^\)]+\s*-\s*mto\s*\)", "SUM(mto)"),
                    ]
            ...
            # 3b) If selecting mto without aggregation while grouping, wrap occurrences in SELECT with SUM()
            if " group by " in updated.lower():
                try:
                    m_select = re.search(r"(?is)\bselect\b", updated)
                    m_from = re.search(r"(?is)\bfrom\b", updated)
                    if m_select and m_from and m_select.end() < m_from.start():
                        head = updated[:m_select.end()]
                        select_body = updated[m_select.end():m_from.start()]
                        tail = updated[m_from.start():]
                        # Replace bare mto in the SELECT body unless already inside SUM()
                        select_body_new = re.sub(r"(?<!SUM\()\bmto\b", "SUM(mto)", select_body, flags=re.IGNORECASE)
                        updated = head + select_body_new + tail
                except Exception:
                    pass

            # 3c) Defensive: if an empty SUM() slipped through, replace it with SUM(mto) when NLQ indicates target-miss semantics,
            # otherwise replace with SUM(0) to avoid SQL binder errors.
            try:
                if re.search(r"(?is)SUM\s*\(\s*\)", updated):
                    if "mto" in table_cols and any(k in nlq_lower for k in ["missed target", "target missed", "target gap", "gap to target", "mto"]):
                        updated = re.sub(r"(?is)SUM\s*\(\s*\)", "SUM(mto)", updated)
                    else:
                        updated = re.sub(r"(?is)SUM\s*\(\s*\)", "SUM(0)", updated)
            except Exception:
                pass
            ...
            # 4b) Deduplicate conflicting region filters (equality vs LOWER equality; alias-qualified vs unqualified)
            try:
                updated = self._dedupe_region_filters(updated)
            except Exception:
                pass

            # 4c) Normalize misplaced AND/WHERE artifacts that can occur after filter removals
            try:
                updated = self._normalize_where_clauses(updated)
            except Exception:
                pass
    ...
    def _ensure_region_filter(self, nlq: str, sql: str) -> str:
        """Inject region filter into SQL if NLQ mentions a SPECIFIC region and SQL lacks a region filter."""
        try:
            nlq_lower = nlq.lower()
            
            # IMPORTANT: Do NOT add region filters for general region queries like:
            # - "top 5 regions", "all regions", "highest sales by region"
            # Only add filters when a SPECIFIC region is mentioned
            ...
            # BUT only if it's clear the user wants a specific region, not a general region query
            partial_match = re.search(r"\b(north|south|central|east|west)\b", nlq_lower)
            if partial_match:
                # Check if this is a general region query (e.g., "top regions", "all regions")
                if re.search(r"\b(top|all|list|show|get)\s+\w*\s*region", nlq_lower):
                    # This is a general region query, don't add filters
                    return sql
    ...
    def _rewrite_growth_to_latest(self, sql: str, table_name: str) -> str:
        ...
        try:
            ...
            # Do not override explicit year constraints if the SQL already contains a year filter
            if not self._sql_has_year_filter(updated):
                # No year filter present; we will inject a proper time clause later
                pass

            # If no time filters present after replacement, inject a precise two-period filter (handle cross-year)
            if not self._sql_has_time_filters(updated):
                # Build (year=prev AND month=prev_m) OR (year=latest AND month=latest_m)
                clause = f"((year = {y0} AND month = {m0}) OR (year = {y1} AND month = {m1}))"
                updated = self._inject_filter_clause(updated, clause)
    ...
    def _rewrite_growth_to_specific(self, sql: str, table_name: str, latest_year: int, latest_month: int) -> str:
        ...
        try:
            ...
            # Do not override an explicit year in SQL; inject only if missing. Handle cross-year properly.
            if not self._sql_has_year_filter(updated):
                clause = f"((year = {prev_year} AND month = {prev_month}) OR (year = {latest_year} AND month = {latest_month}))"
                updated = self._inject_filter_clause(updated, clause)
            return updated
        except Exception as e:
    ...
    def _strip_unintended_region_filter(self, nlq: str, sql: str) -> str:
        """Remove any region filter from SQL if the original NLQ did not explicitly mention a SPECIFIC region."""
        try:
            nlq_lower = nlq.lower()
            
            # Check if NLQ explicitly mentions a SPECIFIC region (not just the word "region" or "regions")
            # We need to distinguish between:
            # - "top 5 regions" (general query about regions) -> NO region filter
            # - "sales in Central-A" (specific region) -> KEEP region filter
            
            # Pattern 1: Exact region names like Central-A, North B, South-A, etc.
            exact_region_match = re.search(r"\b(north|south|central|east|west)[\-\s]?([ab])\b", nlq_lower)
            
            # Pattern 2: Partial region names like "Central region", "North area", etc.
            partial_region_match = re.search(r"\b(north|south|central|east|west)\s+(region|area|territory)\b", nlq_lower)
            
            # Pattern 3: Abbreviated forms like "Central reg", "North reg", etc.
            abbrev_region_match = re.search(r"\b(north|south|central|east|west)\s+reg\b", nlq_lower)
            
            # If any specific region is mentioned, keep the filter
            if exact_region_match or partial_region_match or abbrev_region_match:
                return sql
            
            # If NLQ only contains generic words like "region", "regions" without specifying which one,
            # then remove any region filters from SQL
            if re.search(r"\bregion\b", nlq_lower) and not (exact_region_match or partial_region_match or abbrev_region_match):
                ...
                logger.info("Removed unintended region filter from SQL - NLQ only mentions regions in general, not a specific region")
    ...
    def _strip_unintended_dimension_filters(self, nlq: str, sql: str, table_name: str) -> str:
        """Remove filters for dimensions not explicitly mentioned in the NLQ to avoid hallucinated constraints.
        Dimensions considered: city, area, territory, distributor, brand, sku, customer
        """
        try:
            nlq_lower = nlq.lower()
            dims = ["city", "area", "territory", "distributor", "brand", "sku", "customer"]
            # If any dimension keyword is mentioned in NLQ, we keep filters as-is
            if any(re.search(rf"\b{d}\b", nlq_lower) for d in dims):
                return sql

            # If user mentions a known value (e.g., a city name) present in the table's sample_data or column values,
            # we should respect it as an explicit filter. Collect known values from table schema/sample.
            try:
                table_info = self.db_manager.table_schemas.get(table_name, {})
                sample_rows = table_info.get("sample_data", []) or []
                known_values = set()
            ...
            # If NLQ contains any known value token, consider dimension explicitly mentioned
            for val in known_values:
                if val and re.search(rf"\b{re.escape(val)}\b", nlq_lower):
                    return sql

            # Otherwise, remove simple equality/LIKE filters for these dims from WHERE
            updated = sql
            alias_opt = r"(?:[A-Za-z_][A-Za-z0-9_]*\.)?"
            for d in dims:
                # remove patterns like AND s.city = 'X', AND LOWER(s.city) = 'x', AND s.city IN ('a','b'), AND s.city LIKE '...'
                updated = re.sub(rf"(?is)\s+AND\s+{alias_opt}{d}\s*=\s*'[^']+'", "", updated)
                updated = re.sub(rf"(?is)\s+AND\s+LOWER\(\s*{alias_opt}{d}\s*\)\s*=\s*'[^']+'", "", updated)
                updated = re.sub(rf"(?is)\s+AND\s+{alias_opt}{d}\s+IN\s*\([^)]*\)", "", updated)
                updated = re.sub(rf"(?is)\s+AND\s+{alias_opt}{d}\s+LIKE\s*'[^']+'", "", updated)
                # Remove WHERE variants
                updated = re.sub(rf"(?is)\s+WHERE\s+{alias_opt}{d}\s*=\s*'[^']+'\s+AND", " WHERE", updated)
                updated = re.sub(rf"(?is)\s+WHERE\s+LOWER\(\s*{alias_opt}{d}\s*\)\s*=\s*'[^']+'\s+AND", " WHERE", updated)
                updated = re.sub(rf"(?is)\s+WHERE\s+{alias_opt}{d}\s*=\s*'[^']+'\s*$", "", updated)
            # Cleanup leftover WHERE artifacts
            updated = re.sub(r"\s+WHERE\s+AND", " WHERE", updated)
            updated = re.sub(r"\s+WHERE\s*$", "", updated)
            return updated
        except Exception:
            return sql

    def _reconcile_filters_with_intent(self, nlq: str, sql: str, table_name: str) -> str:
        """Generic filter reconciliation using parsed intent:
        - Adds explicit filters from NLQ (region/city/etc., time) if missing
        - Removes hallucinated dimension filters not present in NLQ intent
        - Keeps SQL structure stable and avoids duplicating WHERE
        """
        try:
            table_cols = [c.lower() for c in self.db_manager.table_schemas.get(table_name, {}).get("columns", [])]
            intent = self._extract_intent_from_nlq(nlq, table_cols, table_name)
            ...
            s = sql
            # Remove hallucinated dimension filters (city/area/territory/distributor/brand/sku/customer)
            # unless the value is explicitly mentioned in NLQ intent
            keep_values = {(c, v) for (c, op, v) in intent.get("filters", []) if isinstance(v, str)}
            dims = ["city","area","territory","distributor","brand","sku","customer"]
            alias_opt = r"(?:[A-Za-z_][A-Za-z0-9_]*\.)?"
            for d in dims:
                # If NLQ did not mention this dimension or a value for it, strip its equality/LIKE filters
                mentioned = any(c == d for (c, _, _) in intent.get("filters", []))
                if not mentioned:
                    s = re.sub(rf"(?is)\s+AND\s+{alias_opt}{d}\s*=\s*'[^']+'", "", s)
                    s = re.sub(rf"(?is)\s+AND\s+LOWER\(\s*{alias_opt}{d}\s*\)\s*=\s*'[^']+'", "", s)
                    s = re.sub(rf"(?is)\s+AND\s+{alias_opt}{d}\s+IN\s*\([^)]*\)", "", s)
                    s = re.sub(rf"(?is)\s+AND\s+{alias_opt}{d}\s+LIKE\s*'[^']+'", "", s)
                    s = re.sub(rf"(?is)\s+WHERE\s+{alias_opt}{d}\s*=\s*'[^']+'\s+AND", " WHERE", s)
                    s = re.sub(rf"(?is)\s+WHERE\s+LOWER\(\s*{alias_opt}{d}\s*\)\s*=\s*'[^']+'\s+AND", " WHERE", s)
                    s = re.sub(rf"(?is)\s+WHERE\s+{alias_opt}{d}\s*=\s*'[^']+'\s*$", "", s)

            # Ensure region filter if intent specifies a specific region
            region_filter = next(((c, op, v) for (c, op, v) in intent.get("filters", []) if c == "region"), None)
            if region_filter and not re.search(r"(?is)\bregion\s*(=|like)\b", s):
                col, op, val = region_filter
                clause = f"LOWER(region) = '{val.lower()}'" if op == "=" else f"region LIKE '{val}'"
                s = self._inject_filter_clause(s, clause)

            # Ensure time filters from intent if present and missing
            year = intent.get("year")
            months = intent.get("months") or []
            if (year is not None or months) and not self._sql_has_time_filters(s):
                s = self._inject_time_filter_into_sql(s, year=year if year is not None else 0, months=months)

            # Normalize WHERE artifacts
            s = self._normalize_where_clauses(s)
            return s
        except Exception:
            return sql
...
LIMIT 1;"""
    ...
    # ---------------- Additional helpers (sanitization & conditional filters) ---------------
    def _maybe_add_route_exclusion(self, nlq: str, sql: str) -> str:
        """Add `route <> 'UNK'` only if the NLQ explicitly mentions a location dimension.
        We check for explicit mentions of region/city/area/territory/distributor/route.
        Avoid adding if already present or if NLQ doesn't mention any location to keep global totals intact.
        """
        try:
            nlq_lower = nlq.lower()
            location_terms = ["region", "city", "area", "territory", "distributor", "route"]
            if not any(term in nlq_lower for term in location_terms):
                return sql
            if re.search(r"(?is)route\s*<>\s*'unk'", sql):
                return sql
            # Inject route <> 'UNK' near existing WHERE or create new WHERE before GROUP/ORDER/LIMIT
            tokens = [" GROUP BY ", " ORDER BY ", " LIMIT ", ";"]
            insert_pos = len(sql)
            lower = sql.lower()
            for t in tokens:
            ...
            tail = sql[insert_pos:]
            if re.search(r"(?is)\bwhere\b", head):
                # Append with AND inside existing WHERE segment (before group/order/limit)
                # Find end of WHERE conditions
                return re.sub(r"(?is)\bwhere\b", "WHERE", head, count=1).rstrip() + " AND route <> 'UNK'" + tail
            else:
                # Create a WHERE
                head = head.rstrip('; ')
                return head + " WHERE route <> 'UNK'" + tail
        except Exception:
            return sql

    def _rewrite_growth_divisions_safe(self, sql: str) -> str:
        """Find growth percentage expressions of the form ((num) * 100.0 / NULLIF(den,0)) AS alias
        or (num / den) * 100 AS alias and wrap them with CASE WHEN den = 0 OR den IS NULL THEN 0 ELSE ... END
        to prevent NaN/Inf leaking to the client. Best-effort regex; leaves original if parse fails.
        """
        try:
            # Pattern 1: ((num) * 100.0 / NULLIF(den,0)) AS alias
            pattern = re.compile(r"\(\(([^()]+)\)\s*\*\s*100\.0\s*/\s*NULLIF\(([^()]+),\s*0\)\)\s+AS\s+([A-Za-z_][A-Za-z0-9_]*|\"[^\"]+\")", re.IGNORECASE)
            def repl(m: re.Match) -> str:
                num = m.group(1).strip()
                den = m.group(2).strip()
                alias = m.group(3)
                safe = (f"CASE WHEN ({den}) = 0 OR ({den}) IS NULL THEN 0 "
                        f"ELSE (({num}) * 100.0 / NULLIF({den},0)) END AS {alias}")
                return safe
            new_sql = pattern.sub(repl, sql)
            # Pattern 2: ((num) / den) * 100 AS alias
            pattern2 = re.compile(r"\(\(([^()]+)\)\s*/\s*([^()]+)\)\s*\*\s*100(?:\.0)?\s+AS\s+([A-Za-z_][A-Za-z0-9_]*|\"[^\"]+\")", re.IGNORECASE)
            def repl2(m: re.Match) -> str:
                num = m.group(1).strip()
                den = m.group(2).strip()
                alias = m.group(3)
                safe = (f"CASE WHEN ({den}) = 0 OR ({den}) IS NULL THEN 0 "
                        f"ELSE (({num}) / NULLIF({den},0)) * 100 END AS {alias}")
                return safe
            new_sql = pattern2.sub(repl2, new_sql)
            return new_sql
        except Exception:
            return sql

    def _sanitize_result_dataframe(self, df):
        """Replace NaN/Inf with None for safe JSON serialization and downstream summarization.
        Also round extremely small floating noise to 0.0 to avoid -0.0 displays and coerce numpy scalars.
        """
        try:
            if df is None or df.empty:
                return df
            sanitized = df.copy()
            # Replace inf/-inf globally, regardless of dtype
            try:
            ...
            for col in sanitized.columns:
                ...
                # Handle float-like columns
                if pd.api.types.is_float_dtype(series):
                    # Growth/percentage columns: treat NA as 0.0
                    if re.search(r"(?i)growth|percentage|percent|ratio|rate", str(col)):
                        series = series.fillna(0.0)
                    # Normalize tiny noise and -0.0; handle numpy floats too
                    series = series.map(
                        lambda v: 0.0
                        if isinstance(v, (float, np.floating)) and not math.isnan(v) and abs(v) < 1e-12
                        else v
                    )
                    sanitized[col] = series
                elif series.dtype == object:
                    # Convert numpy scalar types to native Python and clean invalids
                    def _clean_obj(v):
                        try:
                            if v is None:
                                return None
                            ...
                            return v
                    sanitized[col] = series.map(_clean_obj)

            # Replace remaining NaN/NA with None for JSON safety
            try:
                sanitized = sanitized.where(~sanitized.isna(), None)
            except Exception:
                # Fallback per-cell cleanup
                sanitized = sanitized.applymap(lambda x: None if x is pd.NA or (isinstance(x, float) and (math.isnan(x) or math.isinf(x))) else x)
            return sanitized
        except Exception:
            return df

    def _fix_incorrect_self_joins(self, sql: str, table_name: str) -> str:
        """Remove redundant self-joins on the same table and consolidate aliases to avoid ambiguity.

        Many LLM-generated queries unnecessarily self-join the sole facts table (e.g., "FROM sales_data s JOIN sales_data b ON s.sku=b.sku")
        just to reference another column via a second alias. This causes ambiguous unqualified columns like "region" and often errors.

        Strategy:
        - Detect JOINs where both sides reference the same base table (table_name)
        - Keep the first alias after FROM as the canonical alias (or no alias if none was used)
        - Replace all occurrences of the secondary alias with the canonical alias
        - Remove the redundant JOIN ... ON ... segment entirely (up to next clause boundary)
        - Repeat while such self-joins are found
        """
        try:
            if not sql or table_name is None:
                return sql

            # Work repeatedly in case of multiple self-joins
            while True:
                # Find FROM <table> <alias1> <optional> JOIN <table> <alias2> ON ...
                # Capture the two aliases to be able to replace references
                from_join_pattern = (
                    rf"(?is)\bfrom\s+{re.escape(table_name)}\s+(?:as\s+)?([a-zA-Z_][a-zA-Z0-9_]*)\s+"
                    rf"(?:(?:inner|left|right|full|cross)\s+)?join\s+{re.escape(table_name)}\s+(?:as\s+)?([a-zA-Z_][a-zA-Z0-9_]*)\s+on\b"
                )

                m = re.search(from_join_pattern, sql)
                if not m:
                    # Also handle aliasless FROM joined with alias on the right
                    alt_pattern = (
                        rf"(?is)\bfrom\s+{re.escape(table_name)}\s+"
                        rf"(?:(?:inner|left|right|full|cross)\s+)?join\s+{re.escape(table_name)}\s+(?:as\s+)?([a-zA-Z_][a-zA-Z0-9_]*)\s+on\b"
                    )
                    m_alt = re.search(alt_pattern, sql)
                    if not m_alt:
                        break
                    canonical_alias = None  # aliasless canonical
                    secondary_alias = m_alt.group(1)
                    # Remove the JOIN ... ON ... segment for the detected secondary alias
                    join_segment_pattern = (
                        rf"(?is)\s+(?:(?:inner|left|right|full|cross)\s+)?join\s+{re.escape(table_name)}\s+(?:as\s+)?{re.escape(secondary_alias)}\s+on\s+.*?(?=\s+(?:join|where|group\s+by|having|qualify|order\s+by|limit|;)|$)"
                    )
                    sql = re.sub(join_segment_pattern, " ", sql)
                    # Replace secondary alias references (e.g., b.) with base table name (or nothing if aliasless is supported)
                    # Convert "secondary_alias." -> ""
                    sql = re.sub(rf"(?is)\b{re.escape(secondary_alias)}\.", "", sql)
                    # Clean extra spaces
                ...
                alias1 = m.group(1)
                alias2 = m.group(2)
                canonical_alias = alias1
                secondary_alias = alias2

                # Remove the JOIN ... ON ... segment that introduces the secondary alias
                join_segment_pattern = (
                    rf"(?is)\s+(?:(?:inner|left|right|full|cross)\s+)?join\s+{re.escape(table_name)}\s+(?:as\s+)?{re.escape(secondary_alias)}\s+on\s+.*?(?=\s+(?:join|where|group\s+by|having|qualify|order\s+by|limit|;)|$)"
                )
                sql = re.sub(join_segment_pattern, " ", sql)

                # Replace all references to the secondary alias with the canonical alias
                sql = re.sub(rf"(?is)\b{re.escape(secondary_alias)}\.", f"{canonical_alias}.", sql)
